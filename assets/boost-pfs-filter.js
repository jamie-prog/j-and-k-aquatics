...
var boostPFSFilterConfig = {
	...
	selector: {
        ...
        otpTopCartCount: '[data-cart-count]'
	}
};

var customPricingDataEl = document.getElementById("customer-pricing-data"); 
var CustomPricingData;
if(customPricingDataEl){
  var CustomPricingData = JSON.parse(document.getElementById("customer-pricing-data").text);
}
...
(function() {
    ...
    var selectedVariant = data.variants.filter((variant) =>{
      return variant.id == data.variant_id;
    });
    ...
    // Get Template
		...
		itemHtml = itemHtml.replace(/{{vid}}/g, selectedVariant.id);
		...
		// Add Price
		itemHtml = itemHtml.replace(/{{itemPrice}}/g, buildPrice(data, customerLevelDiscount));
        itemHtml = itemHtml.replace(/{{itemSKU}}/g, buildSKU(data));
        // itemHtml = itemHtml.replace(/{{itemStock}}/g, boostPFSAppConfig.general.template == 'collection.order-form' ? '' : buildStock(data, selectedVariant));
    ...
    // Quantity input
	var itemQuantityHtml = '';
	if (selectedVariant.inventory_policy != "deny" || selectedVariant.inventory_quantity > 0) {
		itemQuantityHtml += `<quantity-input class="quantity-select" data-max="${selectedVariant.inventory_quantity}">`;
		itemQuantityHtml += '<button class="qty-minus c-pointer no-js-hidden" type="button" name="minus">-</button>';
		itemQuantityHtml += '<input class="quantity__input" type="number" name="quantity" value="0" min="0" max="' + selectedVariant.inventory_quantity + '" data-id="' + data.variant_id + '">';
		itemQuantityHtml += '<button class="qty-plus c-pointer no-js-hidden" type="button" name="plus">+</button>';
		if(customerLevelDiscount){ 
			itemQuantityHtml += `<input type='hidden' name='properties[__product-discount3]' value='${customerLevelDiscount.disc3}'><input type='hidden' name='properties[__product-discount5]' value='${customerLevelDiscount.disc5}'><input type='hidden' name='properties[__product-discount6]' value='${customerLevelDiscount.disc6}'>`;
		}
		itemQuantityHtml += '</quantity-input>';
		itemHtml = itemHtml.replace(/{{itemStock}}/g, '');
	} else {
		itemHtml = itemHtml.replace(/{{itemStock}}/g, buildStock(data, selectedVariant));
	}
	itemHtml = itemHtml.replace(/{{itemQuantity}}/g, itemQuantityHtml);
    
    // if (boostPFSAppConfig.general.template == 'collection.order-form') {
    //   if (selectedVariant.inventory_policy != "deny" || selectedVariant.inventory_quantity > 0) {
    //     itemQuantityHtml += `<quantity-input class="quantity-select" data-max="${selectedVariant.inventory_quantity}">`;
    //     itemQuantityHtml += '<button class="qty-minus c-pointer no-js-hidden" type="button" name="minus">-</button>';
    //     itemQuantityHtml += '<input class="quantity__input" type="number" name="quantity" value="0" min="0" max="' + selectedVariant.inventory_quantity + '" data-id="' + data.variant_id + '">';
	...
	function buildPrice(data, customerLevelDiscount) {
		var html = '';
    var selectedVariant = data.variants.filter((variant) =>{
        return variant.id == data.variant_id;
    });
    var selectedVariant = selectedVariant[0];
      ...
      if(customerLevelDiscount){
        var customerLevelDiscountAmount = customerLevelDiscount.discount_amount;
      }
      ...
      //   }
      //   html += '<span class="boost-pfs-filter-product-item-regular-price">' + Utils.formatMoney(data.variants[0].price) + '</span>';
      // }
			html += '</p>';
		}
		return html;
  ...
  function buildSKU(data) {
    ...
    if (data.skus.length >= 1) {
      html = '<p class="variant-sku" allla> SKU: ' + data.skus[0] + '</p>';
    }
		return html;
	}

  function buildStock(data, selectedVariant) {
	var html = '';

	if(selectedVariant.inventory_quantity == 0){
		html = '<span class="low-stock"> Out of Stock</span>'
	}

	else if(selectedVariant.inventory_quantity >= 1){
		html = '<span class="high-stock"> In stock</span>'
	}
	...
	ProductSorting.prototype.bindEvents = function() {
		var _this = this;
		jQ('.boost-pfs-filter-filter-dropdown a').on('click', function(e) {
			e.preventDefault();
		...
		jQ(".boost-pfs-filter-custom-sorting > button").on('click', function() {
		...
		jQ(Selector.filterTreeMobileButton).on('click', function() {
    ...
    BoostOTP.AjaxCart.prototype.onSucess = function(data, $addingLabel, $errorLabel) {
      ...
      jQ.ajax({
        ...
        dataType: 'json',
        success: (data) => {
          jQ('[href="/cart"]').trigger('click')
          bcArnApp.updateCart(data)
    
        }
      });
      
    }
	BoostOTP.QuickView.prototype.onClickAddToCart = function(e) {
...
// 		} else {
// 			quantity = Utils.stripHtml(jQ('#boost-pfs-quickview-cart-quantity').val());
// 			$addingLabel = jQ('#boost-pfs-quickview-cart-btn-text');
// 			$errorLabel = jQ('#boost-pfs-addtocart-error-label');
// 		}
// 		if (selectedVariant) {
// 			AjaxCart.instance.addToCart(selectedVariant.id, quantity, $addingLabel, $errorLabel, true);
// 		}
	}
  
	BoostOTP.QuickView.prototype.onChangeVariant = function() {
		var selectedVariant = this._getSelectedVariant();
	  jQ(".boost-pfs-modal-content .variant-sku").text("SKU: " + selectedVariant.sku)
		if (this.isGetQuickViewOption) {
			this._updateQuickViewOption(selectedVariant)
		} else {
			this._updateQuickViewModal(selectedVariant);
			if (selectedVariant != null && selectedVariant.image) {
				var imageUrl = Utils.optimizeImage(selectedVariant.image, 'small').replace('https:', '').split('?')[0];
				jQ('.boost-pfs-quickview-slider-dot[data-image^="' + imageUrl + '"]').click();
			}
		}
	...
	BoostOTP.QuickView.prototype._updateQuickViewModal = function(selectedVariant) {
	  var variantsMetaData = JSON.parse(document.getElementById("variant-metafields--data").innerHTML);
		// Update prices, image and selected option
		if (selectedVariant) {
			// Get Matched metadata
			let matchedMetaData = variantsMetaData.find((metaData) => {
				return metaData.id == selectedVariant.id;
			})
			// Update Sku;
			if(selectedVariant.sku){
				jQ(".product-sku [data-item-sku]").html(selectedVariant.sku);
				jQ(".product-sku").show();
			}else{
				jQ(".product-sku").hide();
			}
			...
			if(selectedVariant.barcode){
				jQ("[item-barcode]").html(selectedVariant.barcode);
				jQ(".product-barcode").show();
			}else{
				jQ(".product-barcode").hide();
			}
			// update RRP
			if(matchedMetaData && matchedMetaData.rrpgbp){
				jQ("[item-rrp]").html("£" + parseFloat(matchedMetaData.rrpgbp).toFixed(2));
				jQ(".product-rrp").show();
			}else{
				jQ(".product-rrp").hide();
			}
			...
			var $oldOption = $selectElement.find("option[selected]");
			var $newOption = $selectElement.find('option[data-variant-id="' + selectedVariant.id + '"]');
			if($(".boost-pfs-quickview-price-wrapper").find(".discount-notify").length > 0){
				...
				$(".boost-pfs-quickview-price-wrapper").find(".dsc_final_price").html("£" + newDiscountedPrice);
			}

			// Select new
			$selectElement.val(selectedVariant.id);
			if ($oldOption) $oldOption.removeAttr('selected');
			if ($newOption) $newOption.attr('selected', 'selected');

			// Get the price labels
			var priceHtml = $newOption.data('current-price');
			var compareAtPriceHtml = $newOption.data('was-price');

			// Check if onsale or not
			var price = $newOption.data('current-price-without-currency');
			if (price && typeof price == 'string') {
				price = price.replace(/[ ,.]/g, "");
			}
			var compareAtPrice = $newOption.data('was-price-without-currency');
			...
			if (onSale) {
				jQ('#boost-pfs-quickview-current-price').html(Utils.stripHtml(priceHtml));
				jQ('#boost-pfs-quickview-was-price').html(Utils.stripHtml(compareAtPriceHtml));
